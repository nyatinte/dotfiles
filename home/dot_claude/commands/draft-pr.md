...
#### 記載すべき内容
- **背景・目的**: なぜこの変更が必要なのか
  - 例: 「〇〇のパフォーマンス改善のため」「△△との互換性を保つため」「XXにより不要になったため」
- **レビュー観点**: レビュワーに特に見てほしいポイント（あれば）
- 背景情報が汲み取れない場合は、AskUserQuestionを使用してユーザーに尋ねる

#### 記載不要な内容（レビュワーの負担軽減）
- **テスト項目**: CIで確認可能
- **変更ファイル一覧**: GitHubのFiles changedで確認可能
- **コードの詳細説明**: diffを見ればわかる内容
...
もし1.が選択された場合、そのまま`/create-pr`コマンドを実行すること。
