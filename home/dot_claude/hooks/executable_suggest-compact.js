...
if (count === threshold) {
  console.error(`[StrategicCompact] ${threshold} tool calls reached - フェーズ移行なら /compact を検討`);
}

if (count > threshold && (count - threshold) % 25 === 0) {
  console.error(`[StrategicCompact] ${count} tool calls - コンテキストが古くなっていれば /compact のタイミング`);
}
