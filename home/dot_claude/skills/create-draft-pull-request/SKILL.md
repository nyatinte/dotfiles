...
3. `.claude/skills/create-draft-pull-request/pull-request-message.md` を参照し、**.claude/PR.md** を Write または Edit で書き出す
   - `.claude/PR.md` の先頭がタイトルになるため、要注意！
   - Auto が false の場合、意図を明確にするために AskUserQuestion によるヒアリングも必要に応じて行う
4. Auto が false の場合、.claude/PR.md の内容を表示し AskUserQuestion でユーザーに確認する
...
5. `Bash(.claude/skills/create-draft-pull-request/scripts/push-and-create-pr.sh <base-branch>)` を実行して push と PR 作成を行う
   - スクリプトは `.claude/PR.md` をデフォルトで読む（パスは引数で上書き可）
