---
allowed-tools: Read(*), Bash(git commit *), Bash(git log *), Bash(git diff *), Bash(git status *), Bash(git branch *), Bash(git add *), Bash(bash */.claude/skills/git-commit/scripts/analyze-git-diff.sh *)
...
1. `Bash(bash .claude/skills/git-commit/scripts/analyze-git-diff.sh)` を実行（必要なら `--no-diff`）
