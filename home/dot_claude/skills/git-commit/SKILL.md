---
allowed-tools: Read(*), Bash(git commit *), Bash(git log *), Bash(git diff *), Bash(git status *), Bash(git branch *), Bash(git add *), Bash(bash *)
...
1. `Bash(bash scripts/analyze-git-diff.sh)` を実行（必要なら `--no-diff`）
