---
allowed-tools: Read(*), Glob(*), Grep(*), Write(.claude/reviews/*), Bash(gh pr view *), Bash(gh pr diff *), Bash(gh issue view *), Bash(bash */.claude/skills/pr-review-handler/scripts/get-review-threads.sh),Bash(mkdir */.claude/reviews/*)
...
2. `.claude/skills/pr-review-handler/scripts/get-review-threads.sh` を実行して、既存のレビューコメントを確認する
...
6. `.claude/skills/pull-request-review/review-guidelines.md` の内容に従ってレビューコメントを作成する
