function repo() {
	...
	repo=$(ghq list -p | grep -v "/worktrees/" | fzf --height 50% --reverse \
      ...
      [ -n "$readme" ] && FORCE_COLOR=1 CLICOLOR_FORCE=1 RUNEWIDTH_EASTASIAN=0 glow --style tokyo-night --width "${FZF_PREVIEW_COLUMNS:-80}" "$readme" 2>/dev/null || echo "No README found"
      ' \
